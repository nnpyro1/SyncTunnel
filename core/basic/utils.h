...
public://以下是纯静态函数
    static QByteArray getSafePassword(const QString &username,const QString &pwd);                          //派生密钥
    static QByteArray encode(const QByteArray &msg,const QString &username,const QString &pwd);             //加密msg并返回密文
    static QByteArray decode(const QByteArray &msg,const QString &username,const QString &pwd);             //解密msg并返回解密后的值
    ...
    static FileByteArray mergeFileCache(QDir folder,QSet<QString> incremental_sync_set=QSet<QString>(),bool c=1);//合并文件
    static void releaseFile(QByteArray msg);                                                                //释放文件
    static void releaseFileCache(FileByteArray msg);                                                        //释放文件
